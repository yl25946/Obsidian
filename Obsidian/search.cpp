...
namespace Search
{
  ...
  bool Thread::usedMostOfTime()
  {
    ...
    if (Threads::getSearchSettings().standardTimeLimit() || Threads::getSearchSettings().movetime)
  ...
  int Thread::getQuietHistory(Position &pos, Move move, SearchInfo *ss)
  {
    ...
    return mainHistory[pos.sideToMove][move_from_to(move)] + (ss - 1)->contHistory[chIndex] + (ss - 2)->contHistory[chIndex] + (ss - 4)->contHistory[chIndex];
  }
  ...
  Score Thread::negamax(Position &pos, Score alpha, Score beta, int depth, bool cutNode, SearchInfo *ss, const Move excludedMove)
  {
    ...
    // Check time
    ++maxTimeCounter;
    if (this == Threads::mainThread() && (maxTimeCounter & 16383) == 0 && usedMostOfTime())
      Threads::stopSearch();
    ...
    if (!IsPV && !excludedMove && ttScore != SCORE_NONE && ttDepth >= depth && (ttBound & boundForTT(ttScore >= beta)) && pos.halfMoveClock < 90) // The TT entry might trick us into thinking this is not a draw
      return ttScore;
    ...
    if (!IsPV && alpha < 2000 && eval < alpha - RazoringDepthMul * depth)
    {
    ...
    if (!IsPV && depth <= RfpMaxDepth && eval < SCORE_TB_WIN_IN_MAX_PLY && eval - std::max(RfpDepthMul * (depth - improving), 20) >= beta)
      return (eval + beta) / 2;

    // Null move pruning. When our evaluation is above beta, we give the opponent
    // a free move, and if we are still better, cut off
    if (!IsPV && !excludedMove && (ss - 1)->playedMove != MOVE_NONE && eval >= beta && pos.hasNonPawns(pos.sideToMove) && beta > SCORE_TB_LOSS_IN_MAX_PLY)
    {
    ...
    if (!IsPV && depth >= 5 && std::abs(beta) < SCORE_TB_WIN_IN_MAX_PLY && !(ttDepth >= depth - 3 && ttScore < probcutBeta))
    {
      ...
      while (move = pcMovePicker.nextMove(false))
      {
        ...
        if (score >= probcutBeta)
        {
          ...
          // Give bonus to probcut fail high move
          int bonus = stat_bonus(depth - 3);
          PieceType captured = piece_type(pos.board[move_to(move)]);
  ...
  moves_loop:
    ...
    while (move = movePicker.nextMove(skipQuiets))
    {
      ...
      if (!IsRoot && bestScore > SCORE_TB_LOSS_IN_MAX_PLY && pos.hasNonPawns(pos.sideToMove))
      {
        ...
        int seeMargin = isQuiet ? lmrDepth * PvsQuietSeeMargin : depth * PvsCapSeeMargin;
        ...
        if (isQuiet && lmrDepth <= FpMaxDepth && !pos.checkers && ss->staticEval + FpBase + FpDepthMul * lmrDepth <= alpha)
          skipQuiets = true;
      }
      ...
      // Singular extension
      if (!IsRoot && ply < 2 * rootDepth && depth >= 6 && !excludedMove && move == ttMove && abs(ttScore) < SCORE_TB_WIN_IN_MAX_PLY && ttBound & TT::FLAG_LOWER && ttDepth >= depth - 3)
      {
        ...
        if (seScore < singularBeta)
        {
          ...
          if (!IsPV && ss->doubleExt <= DoubleExtMax && seScore < singularBeta - DoubleExtMargin)
          {
      ...
      if (depth >= 2 && seenMoves > 1 + 2 * IsRoot)
      {
        ...
        if (score > alpha && reducedDepth < newDepth)
        {
          ...
          int bonus = score <= alpha ? -stat_bonus(newDepth) : score >= beta ? stat_bonus(newDepth)
                                                                             : 0;
    ...
    if (pos.checkers || (bestMove && bestMoveCap) || (bestScore >= beta && bestScore <= ss->staticEval) || (!bestMove && bestScore >= ss->staticEval))
    {
  ...
  void Thread::startSearch()
  {
    ...
    if (this == Threads::mainThread() && BitCount(rootPos.pieces()) <= TB_LARGEST)
    {
    ...
    for (rootDepth = 1; rootDepth <= settings.depth; rootDepth++)
    {
      ...
      if (settings.standardTimeLimit() && rootDepth >= 4)
      {
        ...
        double scoreLoss = (tm4 / 100.0) + (tm5 / 1000.0) * (idPrevScore - score) + (tm6 / 1000.0) * (searchPrevScore - score);
